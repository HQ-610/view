...
router.post('/modify', function(req, res) { // 修改标签
    var result = req.body;
    MongoClient.connect(url, function(err, db) {
        if(err) throw err;
        var dbo = db.db('analysis');
...
router.post('/add', function(req, res) { // 添加情绪分析
    var result = req.body;
    MongoClient.connect(url, function(err, db) {
        if(err) throw err;
        var dbo = db.db('analysis');
...
router.post('/addTab', function(req, res) { // 添加data数据
    var result = req.body;
    dataSource = {
